...
if IS_TRAIN:
    ...
    for i_episode in range(n_episode):
        ...
        for i_step in range(n_step_per_episode):
            ...
            step_return.append(train_reward) ## rewards appended to step_return list
        
        episode_return = np.sum(step_return) #sum up rewards in each episode Append to Episode_Return list 
        Episode_Return.append(episode_return) 
        step_return = []
        episode_return= 0
...
#a=np.array([Episode_Return])
#c=a/100
epi=np.arange(2000)
epi = epi.tolist()
plt.plot(epi,Episode_Return)
